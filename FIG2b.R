...
df <- read.table("./inputs/FastS_table.tsv", sep = "\t", header = T)
head(df)
unique(df$method)
...
df <- df %>%
  mutate(lineage_set = case_when(
    ...
    endsWith(set_description, "A_5") ~ "A_5",
    ...
    endsWith(set_description, "B.1.617.2_4") ~ "B.1.617.2",
    endsWith(set_description, "A_11") ~ "A_11",
    endsWith(set_description, "B.1.617.2_3") ~ "B.1.617.2",
    endsWith(set_description, "BA.2.12") ~ "BA.2.*",
    endsWith(set_description, "BA.1.15") ~ "BA.1.*",
    endsWith(set_description, "BA.1_2") ~ "BA.1.*",
    endsWith(set_description, "BA.1.1_2") ~ "BA.1.*"
  ))
head(df)
dim(df)
#############################
...
lin <- ggplot(list, aes(axis1 = lineage_genome, axis2= lineage_set, y = n)) +
  ...
  #guides(fill=guide_legend(title="S gene completeness")) +
  ggtitle("Fast-S method", subtitle = "")
lin
...
df[which(df$set_description =="A_11"), 13] <- "Correct"
...
dir.create("hedgehog_Fast-S")
dir.create("hedgehog_Fast-S/tables")
dir.create("hedgehog_Fast-S/tables/experimental_samples")
write.table(pcth, "hedgehog_Fast-S/tables/experimental_samples/hedgehog_pct.tsv", sep = "\t", row.names = F, quote = F)
...
incorrecth <- df[which(df$status_hedgehog == "Incorrect"),]
mu <- incorrecth[grepl("B.1.621", incorrecth$genome_lineage),]
mean(mu$completeness)
sd(mu$completeness)
incorrecth <- incorrecth[-which(incorrecth$genome_lineage == "B.1.621"),]
incorrecth <- incorrecth[-which(incorrecth$genome_lineage == "B.1.621.1"),]
...
write.table(df, "hedgehog_Fast-S/tables/experimental_samples/fast_samples_hedgehog_compare.tsv", sep = "\t", row.names = F, quote = F)

library(ggpubr)
figa <- ggarrange(lin, legend ="bottom")
figb <- ggarrange(difd, difc, ncol = 2, labels = c("B", ""))

fig2 <- ggarrange(figa, figb, ncol = 2, labels = c("A"), widths = c(0.6,0.4), heights = c(0.6,0.4), common.legend = T, legend = "bottom")
fig2
...
##Fast sampled reads
####################
merged <- read.table("./inputs/fastS_sampling_complete_table.tsv", sep = "\t", header = T)

#Get omicron samples obtained with the standard amplicon sequencing protocol
merged2 <- merged[-which(merged$seq == "fast-S"),] #remove data from seq191021
...
lin_omicron <- lin_omicron %>%
  mutate(PANGO_lineage = case_when(
    ...
    endsWith(hedgehog_precision, "BA.1.1_2") ~ "Correct",
    endsWith(hedgehog_precision, "BA.1_2") ~ "Correct",
...
omicron1 <- ggplot(lin_omicron, aes(x=sampling, y=depth, color = PANGO_lineage)) + geom_point(alpha = 0.5, size = 3) +
  ...
  scale_y_continuous(limits = c(0, 500)) +  guides(color=guide_legend(title="hedgehog_precision")) +
...
omicron2 <-  ggplot(lin_omicron, aes(x=sampling, y=completeness, color = PANGO_lineage)) + geom_point(alpha = 0.1, size = 3) +
  theme_bw() + theme(panel.border = element_blank(), 
                     ...
                     legend.title = element_text(size = 18), 
                     legend.text = element_text(size = 18)) +
  scale_y_continuous(limits = c(0, 100)) + guides(color=guide_legend(title="hedgehog_precision")) +
  scale_color_manual(values = c("Correct" = "#fcd5ce", "Incorrect" = "#9e0059")) +
  xlab("") + ylab("") + ggtitle("") + labs(color='PANGO lineage')
omicron2


#remove barcodes from seq280422
...
##Prepare depth and S gene completeness table and graphs
...
lin_alfa$PANGO_lineage <- ifelse(grepl("B.1.1.7", lin_alfa$hedgehog_precision, ignore.case = T), "Correct",
...
lin_beta <- lin_beta %>%
  mutate(PANGO_lineage = case_when(
    ...
    endsWith(hedgehog_precision, "A_1") ~ "Incorrect",
    endsWith(hedgehog_precision, "B.1.547") ~ "Incorrect",
  ))
head(lin_beta)
...
lin_gamma <- lin_gamma %>%
  mutate(PANGO_lineage = case_when(
    ...
    endsWith(hedgehog_precision, "A_24") ~ "Incorrect"))
head(lin_gamma)
...
lin_delta <- lin_delta %>%
  mutate(PANGO_lineage = case_when(
    ...
    endsWith(hedgehog_precision, "B.1.617.2_3") ~ "Correct",
    endsWith(hedgehog_precision, "A_1") ~ "Incorrect",
    endsWith(hedgehog_precision, "B.1.551") ~ "Incorrect",
    endsWith(hedgehog_precision, "AY.48") ~ "Correct",
    endsWith(hedgehog_precision, "AY.121_1") ~ "Correct",
    endsWith(hedgehog_precision, "AY.53") ~ "Correct",
    endsWith(hedgehog_precision, "AY.23") ~ "Correct",
...
lin_mu <- lin_mu %>%
  mutate(PANGO_lineage = case_when(
    ...
    endsWith(hedgehog_precision, "AY.30") ~ "Incorrect",
    endsWith(hedgehog_precision, "B.1.617.2_2") ~ "Incorrect",
    endsWith(hedgehog_precision, "P.1_1") ~ "Incorrect",
    endsWith(hedgehog_precision, "A_1") ~ "Incorrect",
    endsWith(hedgehog_precision, "AY.46.2") ~ "Incorrect",
  ))
head(lin_mu)
...
lin_lambda <- lin_lambda %>%
  mutate(PANGO_lineage = case_when(
    endsWith(hedgehog_precision, "C.37.1") ~ "Correct",
    endsWith(hedgehog_precision, "A_5") ~ "Incorrect"))
head(lin_lambda)
...
f1b <- ggarrange(aa, bb, cc, dd, omicron2, ncol = 5, nrow = 1, common.legend = T)
...
S3 <- ggarrange(f1a, f1b, ncol = 1, nrow = 2, common.legend = T, labels = c("A", "B"))
S3

dir.create("./hedgehog_Fast-S/supplementary3")
png("./hedgehog_Fast-S/supplementary3/fast_voc_depth_completness.png", res = 600, height = 25, width = 40, units = "cm")
S3
dev.off()
...
S4 <- ggarrange(f1a, f1b, ncol = 1, nrow = 2, common.legend = T, labels = c("A", "B"))
S4

dir.create("./hedgehog_Fast-S/supplementary4")
png('./hedgehog_Fast-S/supplementary4/fast_novocvoi_depth_completness.png', res = 600, height = 25, width = 40, units = 'cm')
S4
dev.off()
...
dir.create("hedgehog_Fast-S/tables/sampling_fast_hedgehog")
write.table(tab, "hedgehog_Fast-S/tables/sampling_fast_hedgehog/hedgehog_fast_sampling.tsv", sep = "\t", row.names = F, quote = F)
...
write.table(summary_pass_depth, "hedgehog_Fast-S/tables/sampling_fast_hedgehog/hedgehog_depth_sampling.tsv", sep = "\t", row.names = F, quote = F)
write.table(summary_completeness, "hedgehog_Fast-S/tables/sampling_fast_hedgehog/hedgehog_completeness_sampling.tsv", sep = "\t", row.names = F, quote = F)
...
library(ggpubr)
stats <- ggarrange(depth_g, comp_g, ncol = 2, nrow = 1, common.legend = T)
stats
...
cor <- ggplot(tab, aes(x=depth, y=completeness, color = status)) + geom_point(alpha = 0.3, size = 5) +
  ...
  scale_y_continuous(breaks = seq(0, 100, by = 20)) +
  geom_vline(xintercept = 277, linetype = "dashed")+
  geom_hline(yintercept = 80, linetype = "dashed") +
  annotate("text", x=350, y=81, label= "mean completeness") +
  annotate("text", x=278, y=90, label= "mean depth", angle = 90) +
...
fig4 <- ggarrange(stats, cor, ncol = 2, nrow = 1, common.legend = F,
                  labels = c("C", "D"), font.label = list(size = 14, color = "black", face = "bold", family = NULL, position = "top"))
fig4


FIG2f <- ggarrange(fig2, fig4, nrow = 2, widths = c(0.5, 0.5), heights = c(0.5, 0.5))
FIG2f

dir.create("hedgehog_Fast-S/figure2")
png("./hedgehog_Fast-S/figure2/figure2.png", res = 600, height = 35, width = 40, units = 'cm')
FIG2f 
dev.off()

d <- tab %>% filter(completeness > 80)
dd <- tab %>% filter(depth > 277)
table(dd$status)
mean(dd$sampling)
min(dd$sampling)
max(dd$sampling)
...
t
write.table(t, "hedgehog_Fast-S/tables/sampling_fast_hedgehog/quadrant_depth_compl.tsv", sep = "\t", row.names = F, quote = F)
